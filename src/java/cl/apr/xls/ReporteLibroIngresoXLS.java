 ...
 * @author hmardones
 */
@WebServlet(name = "verReporteIngresos", urlPatterns = {"/verReporteIngresos"})
public class ReporteLibroIngresoXLS extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        System.out.println("[ReporteLibroIngresoXLS] - inicio Servlet ReporteLibroIngresoXLS Excel...");
        ...
        try{
            ...
            Workbook workbook = Workbook.getWorkbook(new File(RUTA_REPORTES+ File.separator +"reportePagos.xls"));
            ...
            if(items != null){  
                ...
                for (int i = 0; i < items.size(); i++) {
                  ir = items.get(i);
                    ...
                    label = new Label(++k, 1+i,ir.getCuenta(),formatoTexto); 
                    sheet.addCell(label);
                    
                    label = new Label(++k, 1+i,ir.getNumeroDocumento(),formatoTexto); 
                    sheet.addCell(label);
                    
//                     label = new Label(++k, 1+i,ir.getIdPago().toString(),formatoTexto); 
//                    sheet.addCell(label);
                    
                    label = new Label(++k, 1+i,ir.getConsumoAgua().toString(),formatoTexto); 
                    sheet.addCell(label);
                    
                      label = new Label(++k, 1+i,ir.getCuotaSocial().toString(),formatoTexto); 
                    sheet.addCell(label);
                    
                       label = new Label(++k, 1+i,ir.getInteres().toString(),formatoTexto); 
                    sheet.addCell(label);
                    
                   

                     label = new Label(++k, 1+i,ir.getMultas().toString(),formatoTexto); 
                    sheet.addCell(label);
                    
                  
                    
                     label = new Label(++k, 1+i,ir.getCorteReposicion().toString(),formatoTexto); 
                    sheet.addCell(label);
                    
                     label = new Label(++k, 1+i,ir.getDerechoIncorporacion().toString(),formatoTexto); 
                    sheet.addCell(label);
                    
                     label = new Label(++k, 1+i,ir.getOtrosCobros().toString(),formatoTexto); 
                    sheet.addCell(label);
                    
                    
                     label = new Label(++k, 1+i,ir.getTotalItem().toString(),formatoTexto); 
                    sheet.addCell(label);
